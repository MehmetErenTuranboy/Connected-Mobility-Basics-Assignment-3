...
for obj in data:
    ...
    if prb_id in rtt_dict:
        rtt_dict[prb_id].append(len(obj["result"]))
    else:
        rtt_dict[prb_id] = []

    if prb_id in rtt_dict_x:
        rtt_dict_x[prb_id].append(obj["timestamp"])
    else:
        rtt_dict_x[prb_id] = []
...
for probe_id, value in probe_types.items():
    if probe_id in rtt_dict:
        plt.plot(rtt_dict_x[probe_id], rtt_dict[probe_id], color=np.random.rand(3, ),
                 label=f"{probe_types[probe_id]}-{int(probe_types_average_distance[probe_id])}" + "km")
...
for probe_id1, value1 in rtt_dict.items():
    for probe_id2, value2 in rtt_dict.items():
        ...
        print(f't={t_stat}, p={p}, pairs={probe_types[probe_id1]}-{probe_types[probe_id2]}, '
              f'distances={int(probe_types_average_distance[probe_id1])}-{int(probe_types_average_distance[probe_id2])}')
        tested_or_not[f"{probe_id1}-{probe_id2}"] = ""

plt.xlabel("Timestamp")
plt.ylabel("Number of round trips")
plt.title(f"Number of round trips {datacenter_url} ")
